...
const CHAPTER_INFO = [
  { week: 'Tuần 1', name: 'Giới thiệu Kinh tế học vĩ mô', clo: 'CLO1' },
  { week: 'Tuần 2', name: 'Chỉ số kinh tế vĩ mô', clo: 'CLO2' },
  { week: 'Tuần 3', name: 'Hệ thống tiền tệ', clo: 'CLO4' },
  { week: 'Tuần 4', name: 'Chính sách tài khóa', clo: 'CLO6' },
  { week: 'Tuần 5', name: 'Nền kinh tế mở', clo: 'CLO8' },
  { week: 'Tuần 6', name: 'Mô hình AS-AD', clo: 'CLO5' },
  { week: 'Tuần 7', name: 'Thất nghiệp và Lạm phát', clo: 'CLO7' },
  { week: 'Tuần 8', name: 'Tăng trưởng kinh tế', clo: 'CLO3' },
]
  ...
  return (
    <div>
      <div className="page-header">
        <h1>Tổng quan</h1>
        <p>Chào mừng bạn đến với hệ thống ôn thi Kinh tế Vĩ mô — Final Exam</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
        {/* Weakest chapters */}
        <div className="card">
          <h3 style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--color-primary)', marginBottom: 14 }}>
            📉 Chương yếu nhất
          </h3>
          {weakestChapters.length === 0 ? (
            ...
            <div key={c.week} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--color-border)' }}>
              <span style={{ fontSize: '0.85rem', fontWeight: 500 }}>{c.week}</span>
        ...
        <div className="card">
          ...
          {weakestCLOs.length === 0 ? (
            ...
            <div key={c.clo} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--color-border)' }}>
              <span className="badge badge-blue">{c.clo}</span>
              <span style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)' }}>{c.total} câu</span>
              <span className={`accuracy-pill ${c.accuracy < 50 ? 'low' : c.accuracy < 75 ? 'mid' : 'high'}`}>{c.accuracy}%</span>
            </div>
          ))}
        </div>
      ...
      {/* Chapter overview */}
      <div className="card">
        <h3 style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--color-primary)', marginBottom: 16 }}>
          ...
          {CHAPTER_INFO.map(ch => (
            <div key={ch.week} className="chapter-card" onClick={() => navigate('/chapters')}>
              ...
              <div className="chapter-meta">
                ...
                <span style={{ fontSize: '0.78rem', color: 'var(--color-text-muted)' }}>{qCountByWeek[ch.week] || 0} câu</span>
              </div>
            </div>
          ))}
        </div>
